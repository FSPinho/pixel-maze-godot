...
func _process(delta):
	...
	for i in matrix.height:
		for j in matrix.width:
			...
			if block.visited and not block.instance_mapped:
				var sprite = Sprite.new()
				sprite.texture = BLOCK_STONE_TEXTURE
				sprite.position = self.bounds.position + Vector2(j * cell_size, i * cell_size) + Vector2(cell_size / 2, cell_size / 2)
				sprite.scale = Vector2(cell_size, cell_size) / sprite.texture.get_size()
				sprite.modulate = Color(0.5, 0.5, 0.5)
				self.add_child(sprite)
				block.instance_mapped = sprite
